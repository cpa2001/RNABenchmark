...
ALLOWED_STRATEGIES = {"content_mean", "cls_ln"}
...
def main():
    ...
    for (strategy, lr), by_setting in grouped.items():
        ...
        candidate = {
            ...
            "rows": sorted(
                valid_rows,
                key=lambda row: (row["variant"], row["loop"]),
            ),
        }
    ...
    if winner is None:
        print("No complete pilot candidate found. Finish all 24 pilot jobs before selecting a winner.", file=sys.stderr)
        sys.exit(1)
