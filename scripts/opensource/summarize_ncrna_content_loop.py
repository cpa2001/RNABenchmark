...
def select_from_logs(log_root, variant, loop, strategy):
    ...
    for log_path in glob.glob(os.path.join(log_root, "*.log")):
        ...
        if best_val is None or test_metrics is None:
            continue
        candidates.append({"lr": job_lr, "best_val": best_val})
...
def aggregate_result_paths(paths):
    ...
    return {
        "count": len(rows),
        "mean_test_accuracy": sum(row["metrics"]["eval_accuracy"] for row in rows) / len(rows),
        "mean_test_f1": sum(row["metrics"]["eval_f1"] for row in rows) / len(rows),
...
def winner_final_variant(setting_label):
    if setting_label == "loop1-stage-d":
        return "loop1-stage-d-content-loop-final"
    return "stage-d-100k-content-loop-final"


def content_mean_final_variant(setting_label):
    if setting_label == "loop1-stage-d":
...
def baseline_variant_for(setting_label, family):
    if family == "content_mean_final":
        return content_mean_final_variant(setting_label)
    if family == "cls":
...
def main():
    ...
    parser.add_argument(
        "--baseline_log_root",
        default="",
        help="Optional old frozen-readout sweep log root for validation-based loop1 adaptive baseline selection.",
    )
    ...
    logs_dir = os.path.abspath(os.path.join(results_root, "..", "logs"))
    ...
    for setting in SETTINGS:
        ...
        winner_paths = [
            (
                ...
                build_result_path(
                    results_root,
                    winner_final_variant(setting_label),
                    winner["strategy"],
                    loop_output,
                    winner["lr"],
                    seed,
        ...
        baselines = {}
        for family in ("content_mean_final", "cls", "adaptive"):
        ...
        content_acc, content_f1, content_count, content_fb_batches, content_fb_examples = fmt_metrics(baselines["content_mean_final"])
        ...
        table_rows.append(
            {
                ...
                "content_fb_batches": content_fb_batches,
                "content_fb_examples": content_fb_examples,
                "cls_acc": cls_acc,
                "cls_f1": cls_f1,
                "cls_count": cls_count,
    ...
    print(
        "setting\twinner(strategy={strategy},lr={lr})\twinner_acc\twinner_f1\twinner_n\twinner_fb_batches\twinner_fb_examples\tcontent_mean_acc\tcontent_mean_f1\tcontent_mean_n\tcontent_mean_fb_batches\tcontent_mean_fb_examples\tcls_acc\tcls_f1\tcls_n\tadaptive_acc\tadaptive_f1\tadaptive_n\tadaptive_lr".format(
            **winner
        )
    )
        ...
        print(
            "\t".join(
                [
                    ...
                    row["content_f1"],
                    row["content_count"],
                    row["content_fb_batches"],
                    row["content_fb_examples"],
                    row["cls_acc"],
                    row["cls_f1"],
                    row["cls_count"],
                    row["adaptive_acc"],
